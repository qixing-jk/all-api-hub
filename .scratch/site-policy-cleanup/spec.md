...
- 修正此前“原生工作区字段解码可保留在主页面”的判断：原生执行能力不等于 OpenRouter 展示协议。控制器保留按账户、站点与作用域标识隔离的 AccountKeyScope，页面从真实作用域读取名称；重试目录时同步名称，避免从密钥字段反推作用域。
- 通用 AccountKeyScopeSelector 通过特性内展示入口获取用词；密钥列表按每行提供方解析展示适配器。OpenRouter 保留专用工作区与字段格式，其他提供方使用公共密钥事实，不擅自解释同名字段或套用美元格式。没有保留旧选择器或 workspaceName 兼容导出。
- 模型入口复用模型列表的来源解析；全账户修复入口不再被当前账户的原生密钥能力隐藏。托管匹配仍由现有 AccountToken 路径执行，混合列表保留刷新，并明确提示原生资源密钥暂不参与；未假装实现原生匹配。
- 同步八种语言的通用作用域、卡片和匹配限制文案；不新增 lint。
- 验证：回归覆盖具有独立模型能力的原生账户、无模型能力的账户、混合列表刷新、跨账户相同作用域 ID、作用域改名以及提供方字段隔离。本批修改涉及的 11 个测试文件、159 项测试通过，compile 和 knip 通过；提取器已同步语言资源排序并清除不再使用的 OpenRouter 列表标题。浏览器 E2E 未运行。
