...
可以保存多个 OpenAI 兼容自定义供应商。每个预设记录自定义名称和 Base URL，按名称自然排序显示；选择后使用 New API 的 OpenAI 渠道类型写入，并完整复用批量 Key、模型读取、手动模型、额度、分组、优先级、定时队列、去重和用量记录流程。预设保存在本地状态；服务器版启用 SQLite 时会在多设备间共享。
...
批量写入默认使用“每条 Key 建立独立渠道”。工具会先写入 1 条验证当前站点可用，成功后将剩余 Key 以 AES-GCM 加密放入安全队列，按每分钟 1 条自动写入，避免批量请求触发 New API 的共享管理接口限流。若 New API 返回明确的 429，当前项和尚未提交的 Key 会立即停止请求并进入自动续传；已经成功的项不会重发。程序只在上游给出不超过 5 秒的明确 `Retry-After` 时原地重试一次，否则交给后台队列；未提供等待时间时默认等待 180 秒，可用 `DATAEYESAI_RATE_LIMIT_FALLBACK_SECONDS` 调整。预览只读取当前渠道类型，分组和默认模型会短时缓存；用量自动校准只抽样 3 条并串行执行，避免在上 Key 前耗尽同一 IP 的管理请求额度。也可以选择 New API 原生 `multi_to_single`，一次请求把所有 Key 写入一个多 Key 渠道，由 New API 随机轮询，同时利用日志里的 Key 索引保留单 Key 用量统计。
