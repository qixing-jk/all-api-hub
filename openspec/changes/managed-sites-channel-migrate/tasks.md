...
- [x] 2.2 Add preview-time source-key hydration for providers that can hide keys, reusing the existing New API verification flow and Done Hub detail loading, while marking only the affected rows as blocked when hydration cannot complete.
...
- [x] 3.1 Add migration entry points to row actions and batch actions in the managed-site channels page, including disabled or hidden states when no eligible target managed site is configured.
...
- [x] 4.2 Add or update focused service tests for explicit managed-site target resolution, preview draft generation, provider-specific mapping warnings, key-hydration blockers, and create-only result handling.
- [x] 4.3 Add or extend options-page/component tests for migration entry points, target picker gating, preview behavior, execution summaries, and filtered “migrate all” behavior.
