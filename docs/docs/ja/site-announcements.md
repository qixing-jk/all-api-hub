...
| 通知の対象期間 | デフォルトは 7 日で設定可能です。この期間より前に公開された公告は既読としてリストに保存され、通知されません。サイト初回の取得で返された公告もすべて履歴として扱われ、通知されません。 |
| 上流の既読同期 | 公告ページで既読にするとサイト側にも同期されます。通知送信成功時にも同期するかどうかは設定で切り替えます。 |
| ローカル保持 | 1 サイトあたり最大 100 件の公告記録を保持します。 |
| 手動チェック | **`サイト公告`** ページの **`今すぐチェック`** は、バックグラウンド取得スイッチの影響を受けません。また、手動チェックでは通知されません。 |
...
| Sub2API | アカウントの未読公告エンドポイントを読み取ります | アカウントの未読公告を取得します。公告ページで既読にすると上流にも同期されます。通知成功直後にも同期したい場合（サイト側の未読状態が消えます）は、設定の **`通知後にサイト側で既読にする`** を有効にしてください。 |
...
2 つの場合があります。公告ページで既読にしたとき、または未読の Sub2API 公告を展開したときは、上流の既読状態を同期しようとします。通知成功直後にも同期したい場合は、設定の **`通知後にサイト側で既読にする`** を有効にしてください。自動では行いません。サイト側の未読状態が消えるためです。サイト API が失敗した場合でも、ローカルの既読状態はユーザー操作に従って更新されます。
