...
::: tip 自動認識でセキュリティ確認が必要な場合
新しい New API では Access Token の生成前にサイトでの認証が必要です。**「Access Token を手動で入力」**と表示された場合、検出済みの情報は保持され、Access Token 認証に切り替わります。

拡張機能のポップアップでは **「サイドパネルで続行」** をクリックすると、入力中のフォームを引き継げます。サイドパネルを開けない場合は通常のページで続行します。続いて **「サイトのセキュリティ設定を開く」** を選び、サイトで認証してトークンをコピーし、保持されたフォームに貼り付けて保存してください。
:::
...
5. 次の項目を入力します。項目名をクリックすると、それぞれの説明に移動できます。
   ...
   **5.2 Access Token の確認方法**
   ...
   有効な Access Token の完全な値を保存済みの場合は、そのまま入力できます。保存していない場合は、新しい New API の **「セキュリティ → Access Token」**（通常は `/security`）を開き、トークンを生成してサイトの認証を完了してください。完全なトークンは一度しか表示されないため、すぐにコピーして拡張機能の **「アクセストークン」** 欄に貼り付けます。

   再生成すると古い Access Token は無効になります。古いトークンを使う他のツールも更新してください。ここではアカウント管理用の Access Token を使用します。「トークン管理」の API キーはモデルの呼び出しに使用します。

   旧バージョンでは、次の画像のようにプロフィールページ下部の「セキュリティ」欄に Access Token の入口がある場合が一般的です。
