...
::: warning 認証情報の転送範囲
ブラウザのローカルストレージはデフォルトの保存場所にすぎません。AI クライアントへのエクスポート、セルフホスト型サイトへのインポート、API / CLI 互換性テスト、WebDAV 同期では、認証情報がそれぞれの宛先アドレスに送信されます。信頼できる宛先にのみキーを送信し、不要になったら失効させてください。
:::
