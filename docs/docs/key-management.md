...
- **显示与复制**：显示或复制前会解析完整密钥；一次性密钥创建后应立即保存，之后可能无法恢复。
- **密钥检查（修复）**：检查已保存账号是否有可用密钥；缺少时会尽量自动创建，其他问题会列在下方。
- **验证与导出**：可验证接口与 CLI 兼容性，可保存到 API 凭据库、导出到 AI 工具或提交到自建站点。
...
| OpenRouter 原生密钥 | 使用 Management API；支持工作区、额度、过期时间、停用等字段；创建时可能只显示一次完整密钥。 |
...
- 列表默认用遮罩显示密钥（例如 `sk-***...`）。
- 点击 **`显示密钥`** / **`隐藏密钥`** 可以在页面内临时查看完整值；**`复制`** 会把完整密钥复制到剪贴板。
- 遮罩只保护“显示”，并不代表密钥已加密存储。复制、验证、导出、备份或提交到远端时，都会使用完整密钥值。
- 如果某个密钥是“只创建时显示一次”的类型（例如 AIHubMix、OpenRouter 的部分场景），请创建后立即保存；之后页面可能无法再次显示完整密钥。
...
- **遮罩不等于加密**：页面遮罩只保护显示；复制、验证、导出、备份和远端提交都会使用完整密钥值。
...
| 问题 | 说明 |
|------|------|
| 为什么我看不到完整的密钥？ | 站点后台可能禁用了密钥回显，或者该密钥只在创建时显示一次。可尝试“密钥检查”（需对应权限），但不能保证恢复。 |
| Key 的安全性如何？ | 页面默认遮罩密钥，遮罩只保护显示；复制、验证、导出或远端提交都会使用完整值。是否加密存储按来源而定，不要假设默认加密。 |
