...
- **多策略支持**：可选择“智能合并 / 最大合并 / 仅上传 / 仅下载”，满足主次设备差异化需求。
- **冲突合并**：
  - **智能合并**：按更新时间对同一项去重，保留最新版本。
  - **最大合并**：在保留最新版本的同时填补缺失字段并合并数组，尽量避免数据丢失。
...
| **同步策略** | `merge`（智能合并）、`max_merge`（最大合并，防数据丢失）、`upload_only`（以本地覆盖远程）、`download_only`（以远程覆盖本地）。 |
...
2. 执行同步时：
   ...
   - 导出本地账号与偏好，依据策略决定最终数据：
     - **智能合并**：根据 `updated_at` / `lastUpdated` 时间戳保留最新项。
     - **最大合并**：以最新数据为基底，合并双方缺失字段与数组内容。
     - **仅上传/下载**：直接选取本地或远程数据。
