...
| Execution result says the site type does not match | The extension probed the site after a failed check-in and found a type other than the one the account carries, and the hint names the type to switch to (for example: this site matches Veloera, while the account is set to new-api). Fix: in Account Management → Edit Account, set the Site Type to the named type, then run once more. The hint is shown only while the account still carries the type recorded at the time. |
