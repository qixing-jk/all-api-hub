...
The **Mesh Gradient Debugging Tool** is a built-in visual debugging tool for previewing and customizing the dynamic backgrounds in the [Share Snapshot](./share-snapshot.md) feature.
...
This tool is for development and testing only, so it's not in the regular menus:
