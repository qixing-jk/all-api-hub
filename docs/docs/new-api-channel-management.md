...
在渠道列表的行尾菜单中，点击 **"编辑渠道过滤规则"** 可以为单个渠道配置专属的模型过滤规则。这些规则会在模型同步时自动应用，控制哪些模型会被同步到该渠道。

### 规则类型

#### 1. 模式匹配规则（Pattern）

基于模型名称的字符串匹配或正则表达式过滤。

- **普通模式**：区分大小写的子串匹配，例如 `gpt-4` 会匹配 `gpt-4`、`gpt-4-turbo` 等。
- **正则模式**：使用 JavaScript 正则语法，例如 `^gpt-4.*turbo$` 精确匹配以 `gpt-4` 开头、以 `turbo` 结尾的模型。

#### 2. 探测型规则（Probe）

通过实际 API 调用测试模型能力，根据探测结果过滤模型。支持以下探测类型：

- **模型列表探测**：测试模型是否出现在 API 的模型列表中。
- **文本生成探测**：测试模型是否支持基础的文本生成功能。
- **工具调用探测**：测试模型是否支持 Function Calling / Tool Use。
- **结构化输出探测**：测试模型是否支持 JSON Schema 约束的结构化输出。
- **联网搜索探测**：测试模型是否支持联网搜索功能。

### 探测规则的凭证配置

探测规则需要有效的 API 凭证才能运行。凭证解析优先级如下：

1. **规则级凭证**：在规则中直接填写 Base URL、API 密钥和 API 类型。
2. **渠道级凭证**：在"渠道校验凭证"区域配置的默认凭证（所有探测规则共享）。
3. **渠道自身凭证**：使用渠道的 Base URL 和 Key（如果可用）。

**推荐做法**：

- 从 **API 凭证** 创建渠道时，校验凭证会自动带入，无需手动配置。
- 手动创建的渠道，可在过滤规则对话框顶部的 **"渠道校验凭证"** 区域统一配置，避免为每个探测规则重复填写。
- 如果某个探测规则需要使用不同的凭证（例如测试不同的 API 提供商），可在规则中单独填写。

### 性能注意事项

- 探测规则会对每个模型执行实际的 API 调用，可能较慢且消耗配额。
- **建议先使用模式匹配规则缩小模型范围，再应用探测规则进行精细筛选。**
- 探测过程会自动应用速率限制（与模型同步共享），避免触发 API 限流。

### 使用示例

**场景 1：只同步支持工具调用的 GPT-4 模型**

1. 添加模式匹配规则：`包含` 匹配 `gpt-4`（缩小范围）。
2. 添加探测型规则：`包含` 探测类型为 `工具调用探测`。

**场景 2：排除不支持联网搜索的模型**

1. 添加探测型规则：`排除` 探测类型为 `联网搜索探测`（探测失败的模型会被排除）。
