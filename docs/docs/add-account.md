...
::: tip 自动识别提示安全验证
新版 New API 在生成 Access Token 前要求完成站点安全验证。出现 **“手动填写 Access Token”** 提示时，插件会保留已识别的信息，并切换到访问令牌认证。

如果正在使用扩展弹窗，点击 **“转到侧边栏继续”**，当前表单会一起带过去；无法打开侧边栏时会在完整页面继续。然后点击 **“打开站点安全设置”**，在站点完成验证、复制令牌，再回到保留的表单中填写并保存。
:::
...
5. 在表单中填写以下信息，点击字段名称可以跳转到下方对应的填写说明：
   ...
   **5.2 怎么获取访问令牌**
   ...
   如果你已保存有效的完整 Access Token，可以直接填写。否则，在新版 New API 的 **“安全设置 → Access Token”** 页面（通常为 `/security`）生成令牌，并按站点提示完成安全验证。生成后的完整令牌只显示一次，请立即复制并粘贴到插件的 **“访问令牌”** 字段。

   重新生成会使旧 Access Token 失效，其他使用旧令牌的工具也需要更新。这里填写的是账号管理用的 Access Token；“令牌管理”中的 API Key 用于模型调用。

   旧版页面通常在个人资料页下方的“安全”栏目中提供“访问令牌”入口，如下图所示。
