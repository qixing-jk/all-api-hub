...
2. **手动添加**：在自动识别失败后，按照 [手动添加账号指南](./add-account.md#manual-addition) 选择站点类型，并检查对应字段：
   - New API 及兼容站点：用户名、用户 ID、访问令牌和充值比例
   - Sub2API：用户 ID、JWT 访问令牌和充值比例；开启插件托管会话时还需 Refresh Token
   - OpenRouter：Management Key 和充值比例；无需用户 ID，也不支持 Cookie 模式
...
3. **开启全局计划**：进入「设置 → 签到与兑换 → 自动签到」，确认「启用自动签到」已开启，并按需设置时间窗口、计划模式和重试。
4. **立即测试**：在「自动签到」页点击「立即执行」测试全部账号；只想测试一个账号时，在账号“…”菜单点击「快速签到」。
