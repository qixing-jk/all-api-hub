...
- 暂不支持内置自动签到（仍可为该账号配置外部签到 URL 使用）。
...
2. **手动添加**：在自动识别失败后，按照 [手动添加账号指南](./add-account.md#manual-addition) 选择站点类型，并检查对应字段：
   - New API 及兼容站点：站点名称、用户名、用户 ID、访问令牌和充值比例
   - Sub2API：站点名称、用户 ID、JWT 访问令牌和充值比例；开启插件托管会话时还需 Refresh Token
   - OpenRouter：站点名称、Management Key 和充值比例；无需用户 ID，也不支持 Cookie 模式
   - 其他站点：以表单当前显示的必填标记和对应站点说明为准
...
部分中转站支持每日签到获取奖励，完整说明请查看 [自动签到](./auto-checkin.md)。这里是最短流程：

1. **添加账号并刷新**：在「账号管理」添加账号，并成功刷新/识别一次。
2. **确认账号级开关**：编辑账号，在「签到配置」里确认出现「启用每日自动签到」；看不到通常表示该站点类型不支持内置自动签到。
3. **配置全局计划（可选）**：进入「设置 → 签到与兑换 → 自动签到」，按需开启「启用自动签到」并设置时间窗口、计划模式和重试；关闭全局开关只会停止自动任务，不影响后续手动签到。
4. **立即测试**：在「自动签到」页点击「立即执行」测试全部符合条件的账号；只想测试一个账号时，在账号“…”菜单点击「快速签到」。

注意事项：

- 「快速签到」会真实调用该账号的内置签到提供方。
- 「手动签到」和「外部签到」只是打开页面，需要你在网页里完成签到，不代表已经签到成功。
- 账号表单里没有“签到检测”开关，签到状态在账号刷新/识别时自动读取。
