...
export function PricingScenarioControls({
  ...
  const hasSpecialConditions =
    ...
    taskFields.length > 0 ||
    hasTimeCondition ||
    PRICING_SCENARIO_SELECTION_AXES.some(supportsSelection)
  const conditionFields =
    hasContextCondition || hasSpecialConditions ? (
      <div className="min-w-0 space-y-3">
        <div className="space-y-0.5">
          <h4 className="text-sm font-medium">
            {t("scenario.conditionsTitle")}
          </h4>
          <p className="text-muted-foreground text-xs">
            {hasContextCondition
              ? t("scenario.contextEffect")
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          {PRICING_SCENARIO_SELECTION_AXES.filter(supportsSelection).map(
            (axis) => (
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    onChange({
                      ...settings,
                      [axis]:
                        value === UNSPECIFIED_PRICING_OPTION
                          ? undefined
                          : value,
                    })
                  }
  ...
  const summary = conditionFields ? (
    <div className="text-muted-foreground flex flex-wrap gap-x-4 gap-y-1 text-xs">
      {hasInputContextCondition && hasOutputContextCondition ? (
        <span>
          {t("scenario.contextSummary", {
            input: settings.inputTokens?.toLocaleString() ?? "—",
            output: settings.outputTokens?.toLocaleString() ?? "—",
          })}
        </span>
      ) : hasContextCondition ? (
        <span>
      ...
      {hasTimeCondition && (
        <span>
          {t("scenario.snapshot", {
            time: new Date(settings.at).toLocaleString(),
          })}
        </span>
      )}
    </div>
