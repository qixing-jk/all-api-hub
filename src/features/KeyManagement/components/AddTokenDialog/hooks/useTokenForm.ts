      ...
      if (isEditMode && editingToken) {
        ...
        const normalizedGroup =
          typeof createPrefill?.group === "string"
            ...
            : ""
        const finalGroup = normalizedGroup || initialFormData.group
        
        // Build token name from account name and group
        const defaultAccount = availableAccounts.find(
          (acc) => acc.id === defaultAccountId,
        )
        const autoGeneratedName = defaultAccount
          ? `${defaultAccount.name}-${finalGroup}`
          : ""
